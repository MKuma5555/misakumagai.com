import Link from 'next/link'
import { User, Layers, Folder, Mail, ArrowUpRight } from 'lucide-react'

const items = [
  { id: 'about', label: 'About', Icon: User },
  { id: 'skills', label: 'Skills', Icon: Layers },
  { id: 'works', label: 'Works', Icon: Folder },
  { id: 'contact', label: 'Contact', Icon: Mail },
] as const

// lucide からブランドアイコンが削除されたため、SNSは自前のSVGで持つ
const Github = (p: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="currentColor" className={p.className} aria-hidden="true">
    <path d="M12 .5C5.37.5 0 5.87 0 12.5c0 5.3 3.44 9.8 8.2 11.39.6.11.82-.26.82-.58v-2.03c-3.34.73-4.04-1.61-4.04-1.61-.55-1.39-1.34-1.76-1.34-1.76-1.09-.75.08-.73.08-.73 1.2.08 1.84 1.24 1.84 1.24 1.07 1.83 2.81 1.3 3.5.99.11-.78.42-1.3.76-1.6-2.67-.3-5.47-1.34-5.47-5.96 0-1.32.47-2.39 1.24-3.23-.13-.3-.54-1.53.12-3.18 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 0 1 6.010 0c2.29-1.55 3.3-1.23 3.3-1.23.66 1.65.25 2.88.12 3.18.77.84 1.24 1.91 1.24 3.23 0 4.63-2.8 5.65-5.48 5.95.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.83.58A12.01 12.01 0 0 0 24 12.5C24 5.87 18.63.5 12 .5z" />
  </svg>
)
const Linkedin = (p: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="currentColor" className={p.className} aria-hidden="true">
    <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.42v1.56h.05c.47-.9 1.64-1.85 3.37-1.85 3.6 0 4.27 2.37 4.27 5.46v6.28zM5.34 7.43a2.06 2.06 0 1 1 0-4.13 2.06 2.06 0 0 1 0 4.13zM7.12 20.45H3.55V9h3.57v11.45zM22.22 0H1.77C.79 0 0 .77 0 1.72v20.56C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.72V1.72C24 .77 23.2 0 22.22 0z" />
  </svg>
)
// 未開設のため現在は未使用。開設したら socials に戻す
export const Instagram = (p: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="currentColor" className={p.className} aria-hidden="true">
    <path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23-.22.56-.48.96-.9 1.38-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41a3.8 3.8 0 0 1-1.38-.9 3.8 3.8 0 0 1-.9-1.38c-.16-.42-.36-1.06-.41-2.23C2.17 15.58 2.16 15.2 2.16 12s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41C8.42 2.17 8.8 2.16 12 2.16zM12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63c-.79.3-1.46.72-2.13 1.38A5.9 5.9 0 0 0 .63 4.14c-.3.76-.5 1.64-.56 2.91C.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.3.79.72 1.46 1.38 2.13a5.9 5.9 0 0 0 2.13 1.38c.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56a5.9 5.9 0 0 0 2.13-1.38 5.9 5.9 0 0 0 1.38-2.13c.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91a5.9 5.9 0 0 0-1.38-2.13A5.9 5.9 0 0 0 19.86.63c-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0zm0 5.84a6.16 6.16 0 1 0 0 12.32 6.16 6.16 0 0 0 0-12.32zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm7.85-10.4a1.44 1.44 0 1 1-2.88 0 1.44 1.44 0 0 1 2.88 0z" />
  </svg>
)

export default function SiteFooter({ en, locale }: { en: boolean; locale: string }) {
  // /ja は Instagram を前に（Instagram運用がサービスにあるため自分の運用が実力の証明になる）
  const github = { name: 'GitHub', href: 'https://github.com/MKuma5555', Icon: Github }
  const linkedin = { name: 'LinkedIn', href: 'https://www.linkedin.com/in/misa-k-609305205/', Icon: Linkedin }
  // Instagram は未開設。アカウントを作ったら instagram を配列に戻す
  // const instagram = { name: 'Instagram', href: '', Icon: Instagram }
  const socials = en ? [github, linkedin] : [linkedin, github]

  return (
    <footer className="bg-[#4a5e3e] px-6 py-14 text-[#f4f0e6] md:px-16">
      <div className="mx-auto max-w-[1240px]">
        <Link href={`/${locale}`} className="font-serif text-2xl">
          Misa <span className="text-[#d6dfc9]">.</span>
        </Link>

        <div className="mt-7 flex flex-wrap gap-2.5">
          {items.map(({ id, label, Icon }) => (
            <Link
              key={id}
              href={`/${locale}/#${id}`}
              className="inline-flex items-center gap-2 rounded-full bg-[#f4f0e6]/12 px-4 py-2 text-xs transition-colors hover:bg-[#f4f0e6]/22"
            >
              <Icon size={14} />
              {label}
            </Link>
          ))}
        </div>

        <div className="mt-7 flex gap-4">
          {socials.map(({ name, href, Icon }) => (
            <a
              key={name}
              href={href}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={name}
              className="text-[#d6dfc9] transition-colors hover:text-[#f4f0e6]"
            >
              <Icon className="h-[18px] w-[18px]" />
            </a>
          ))}
        </div>

        <div className="mt-10 flex flex-col justify-between gap-4 border-t border-[#d6dfc9]/20 pt-6 md:flex-row md:items-center">
          <span className="font-mono text-[10px] tracking-wider text-[#d6dfc9]">
            MADE IN AUSTRALIA · FOR EVERYWHERE
          </span>
          <Link href={`/${locale}/#top`} className="flex items-center gap-2 text-sm">
            Back to top <ArrowUpRight size={14} />
          </Link>
        </div>
      </div>
    </footer>
  )
}
